...
\begin{frame}
    \begin{theorem}
        ...
        and Dirac operator $\D$. Then we have for every $a \in C^\infty(M)$,
        \begin{equation*}
            da = [\D,a]
        \end{equation*}
...
\begin{frame}
\frametitle{Differentials in abstract algebra}
\begin{definition}
...
\begin{frame}
\frametitle{Can we do better?}
Similar necessary and sufficient conditions can be found on $f$
such that $\qd f \in \mathcal{L}^{p,q}$.
...
\begin{proofs}
    ...
    \end{align*}
    And define the rank one operator $A_\zeta = y_\zeta(x,x_\zeta)$. 
\end{proofs}
\end{frame}
...
We can now interpret this in a rigorous sense, by replacing infinitesimals
with quantised differentials, and ``ignoring sufficiently small infinitesimals"
means working modulo some operator ideal.
\end{frame}
