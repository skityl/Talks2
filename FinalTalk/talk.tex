...
\begin{frame}
\frametitle{Besov spaces and Sobolev spaces}
\begin{theorem}
    For $1 \leq p \leq 2$, we have $B_{pp}^s(\Circ) \subseteq W_p^s(\Circ)$,
    and for $2 \leq p < \infty$, $W_{p}^s(\Circ) \subseteq B_{pp}^s(\Circ)$.
    
    In particular, $B_{2,2}^s(\Circ) = W_2^s(\Circ)$.
\end{theorem}
\end{frame}
...
\end{theorem}
In particular: we have $\qd f \in \mathcal{L}^2$ if and only
if $f \in W_2^{1/2}(\Circ)$.
\end{frame}
